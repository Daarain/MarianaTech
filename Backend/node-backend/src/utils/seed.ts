...
export async function seedInitialDataIfEmpty() {
  ...
  try {
    ...
    if (missionCount === 0) {
      console.log('[Seed] Populating initial mission database documents...');
      await Mission.insertMany(mockMissions);

      const anomalyDocs = [];
      for (const m of mockMissions) {
        for (let i = 0; i < anomalyTemplates.length; i++) {
          const tmpl = anomalyTemplates[i];
          const anomalyId = `ANM-${m.id.slice(-4)}-${String(i + 1).padStart(3, '0')}`;
          anomalyDocs.push({
            ...tmpl,
            id: anomalyId,
            mission_id: m.id,
      ...
      console.log(`[Seed] Successfully seeded ${mockMissions.length} missions and ${anomalyDocs.length} anomalies into MongoDB.`);
    }
  } catch (err: any) {
    console.warn(`[Seed Warning] Database seeding skipped: ${err.message}`);
  } finally {
    isSeeding = false;
  }
}
