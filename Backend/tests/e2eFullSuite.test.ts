...
import { validateAndSanitizeAnomaly, requestInferenceFromFastAPI } from '../src/services/ai.service';
...
describe('COMPREHENSIVE END-TO-END (E2E) TEST SUITE', () => {
  ...
  beforeAll(async () => {
    ...
    await User.deleteMany({ username: { $in: ['e2e_operator', 'e2e_admin'] } });
    await Mission.deleteMany({ name: /E2E/ });
  ...
  describe('Happy Path Execution Flow', () => {
    ...
    test('STEP 9: Mission Retrieval via GET /missions', async () => {
      ...
      const created = res.body.find((m: any) => m.id === testMissionId);
    ...
    test('STEP 12-14: Asynchronous Processing & BullMQ Job Queueing', async () => {
      ...
      await Mission.findOneAndUpdate({ id: testMissionId }, { status: 'pending' });
      ...
      const job = await ProcessingJobModel.findOne({ jobId: testJobId });
    ...
    test('STEP 19: Anomaly Retrieval via GET /missions/:missionId/anomalies', async () => {
      ...
      const found = res.body.find((a: any) => a.id === testAnomalyId) || res.body[0];
    ...
    test('STEP 25: Audit Trail Verification', async () => {
      const histories = await VerificationHistoryModel.find({ anomalyId: testAnomalyId });
  ...
  describe('Failure Scenarios Execution Flow', () => {
    ...
    test('Failure Scenario 8: FastAPI Microservice Unavailable (Retry & Exception)', async () => {
      ...
      const response = await requestInferenceFromFastAPI({
        jobId: 'JOB-ERR-01',
        missionId: testMissionId,
        fileId: 'FILE-01',
    ...
    test('Failure Scenario 12: Invalid AI Response Sanitization (Confidence Clamping)', () => {
      ...
      const sanitized = validateAndSanitizeAnomaly(invalidAIAnomaly);
    ...
    test('Failure Scenario 13: Duplicate Processing Request Enforcement', async () => {
      ...
      await Mission.findOneAndUpdate({ id: testMissionId }, { status: 'processing' });
    ...
    test('Failure Scenario 15: Unavailable GPS Metadata Handling (GeoJSON Omission)', async () => {
      const noGPSAnomaly = await AnomalyModel.create({
    ...
    test('Failure Scenario 16: Non-Existent Report File Download Request', async () => {
      const res = await request(app).get('/reports/file/RPT-NON-EXISTENT-9999');
      expect(res.status).toBe(404);
